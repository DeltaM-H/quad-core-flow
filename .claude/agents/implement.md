...
- [ ] summary.md 包含 **接口签名**、**安全敏感代码路径**、**设计决策摘要**、**代码质量关注点**、**架构影响分析** 五个段落
- [ ] 每个段落有实质性内容（>50 非空字符）
- [ ] 简略摘要已写入指定路径
...
**### 3. 设计决策摘要（~1K）**
方案选择理由、替代方案评估，用于 Design Reviewer 审查。
...
**### 5. 架构影响分析（~1K）**
分析本次实现对项目架构的影响，用于 Architecture Reviewer 审查。
包括：模块边界变化、新增依赖方向、架构模式一致性、潜在的架构侵蚀风险。
如果影响较小，写明"本次变更局限在单模块内，无架构影响"。
{% endif %}
